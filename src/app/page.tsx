...
import { Shirt, SocialFollow, badgeDot, badgeNeon, badgeRed, button, buttonWhite, headline, subheadline } from "@/components/site/shared";
...
const REVIEWS = [
  { rotate: "-rotate-[4deg]", w: 480, h: 580, aspect: "aspect-[480/580]", badge: `${badgeNeon} left-[-65px] top-[405px] w-1/2 rotate-[14deg]` },
  { rotate: "rotate-0", w: 600, h: 800, badge: `${badgeRed} left-[247px] top-[389px] w-1/2 rotate-[14deg]` },
  { rotate: "rotate-[4deg]", w: 600, h: 800, badge: `${badgeNeon} left-[-49px] top-[333px] w-1/2 rotate-[14deg]` },
];
    ...
    <>
      ...
      <section className="site-container flex flex-row max-lg:flex-col max-md:px-5">
        <div className="flex w-1/2 flex-col gap-5 pt-10 max-lg:w-full max-lg:items-center max-lg:text-center">
          ...
          <p className="max-lg:w-[60%] max-md:w-[80%]">Само за 17.99EU на месец</p>
          <Link href="/join" className={`${button} mt-3.5 self-start max-lg:self-center`}>ВИЖ ТЕНИСКИТЕ</Link>
        </div>

        <div className="relative flex h-[517px] w-1/2 flex-row flex-nowrap items-center max-lg:mt-[60px] max-lg:h-auto max-lg:w-full">
          <Image src="/wp/2025/12/shapeA.svg" alt="" width={640} height={445} className="absolute left-[-10px] top-[26px] z-[1] w-full translate-x-[37px] scale-90 max-lg:left-[-267px] max-lg:top-[6px] max-lg:w-[30%] max-lg:translate-x-[99px] max-md:left-[-150px] max-md:top-[-72px] max-md:w-[88%] max-md:translate-x-[37px] max-md:scale-50" />
          <Image src="/wp/2025/12/shapeB.svg" alt="" width={241} height={237} className="absolute left-[399px] top-[402px] z-[1] w-[241px] translate-x-[37px] max-lg:left-[125px] max-lg:top-[218px] max-lg:w-[29%] max-md:left-[99px] max-md:top-[135px] max-md:w-[381px]" />
          <div className="w-[32%] shrink-0 max-lg:w-[233px] max-md:w-[35%]">
            <Shirt className="-rotate-[9deg] scale-[1.3] max-lg:scale-[0.8]" />
          </div>
          <div className="z-[2] flex-1">
            <Shirt className="scale-[1.5] max-lg:scale-100 max-md:scale-[1.5]" />
          </div>
      ...
      <section
        className="site-container relative mt-40 flex h-[512px] flex-col items-center bg-[url('/wp/2025/12/bg.svg')] bg-[length:96%_auto] bg-center bg-no-repeat max-lg:mt-[120px] max-lg:bg-cover max-md:h-auto"
      >
        <div className="relative z-[2] w-[30%] max-lg:w-[60%] max-md:w-full">
          <div className="pt-[41px]">
            <Shirt className="w-full rotate-[4deg] scale-[0.8] max-lg:scale-[1.1] max-md:scale-[0.75]" />
          </div>
          <div className={`${badgeRed} left-[-148px] top-[306px] w-[257px] rotate-[9deg] max-md:left-[13px] max-md:top-[367px]`}>Удобна за всеки повод</div>
          <div className={`${badgeNeon} left-[279px] top-[217px] w-1/2 -rotate-[4deg] max-md:left-[-9px] max-md:top-[84px]`}>100% Памук</div>
          <div className={`${badgeDot} left-[259px] top-[424px] w-1/2 rotate-[14deg] max-md:left-[110px] max-md:top-[265px]`}>ЕКО Материя</div>
        </div>
      </section>

      {/* USP row: 1140px container, four equal columns */}
      <section className="mx-auto mt-20 flex w-full max-w-[1140px] flex-row pb-[54px] pt-10 max-lg:flex-wrap max-lg:items-center max-lg:justify-center max-lg:gap-[15px] max-md:px-5">
        {USPS.map((u, i) => (
          <div key={i} className="flex flex-1 flex-col items-center max-lg:w-[45%] max-lg:flex-none max-md:w-full">
            <Image src={u.icon} alt="" width={114} height={u.h} className="w-[40%] max-lg:h-[150px] max-lg:w-auto max-md:h-[130px]" />
      ...
      <section className="site-container relative mt-40 h-[81px]">
        <h2 className={`${headline} relative z-[2] text-center max-md:text-[36px]`}>Доволни клиенти</h2>
      ...
      <section className="mx-auto mt-20 flex w-full max-w-[1140px] flex-row gap-20 pb-[27px] max-lg:max-w-[678px] max-md:flex-col max-md:gap-0 max-md:px-5">
        {REVIEWS.map((r, i) => (
          <div key={i} className={`relative z-[2] flex flex-1 flex-col ${r.rotate} max-md:p-5`}>
            ...
            <p className="mt-7">“Перфектна тениска за ежедневието”</p>
          </div>
        ))}
      </section>

      {/* FAQ: red band, 60px padding */}
      <section className="site-container relative my-40 flex flex-col items-center justify-center bg-t-red pb-[74px] pt-[60px] max-lg:px-10 max-md:px-5">
        <h2 className={`${headline} relative z-[2] w-full text-left text-white max-md:text-[36px]`}>Често задавани въпроси</h2>
        <div className="mt-5 flex w-full flex-row gap-20 text-white max-lg:flex-col">
          {FAQ.map((f, i) => (
        ...
        <Image src="/wp/2025/12/star2.svg" alt="" width={200} height={202} className="absolute left-[72px] top-[-77px] z-[1] w-[120px] max-lg:left-[139px] max-lg:top-[-123px] max-md:left-[93px] max-md:top-[-129px]" />
      </section>
      ...
      <section className="site-container my-40 flex flex-row items-center">
        ...
        <div className="flex w-1/2 flex-col items-center max-lg:w-full">
          ...
          <div className="flex h-[121px] w-[80%] items-center justify-center text-center font-dot text-[21px] uppercase tracking-[0.84px] text-t-black max-md:text-[14px]">
            искам нещо лежерно за фитнеса
          </div>
          <Link href="/join" className={button}>ЗАпиши се сега</Link>
        </div>
