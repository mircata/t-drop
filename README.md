...
- The moving strip at the top, the menu, the footer links and the socials: Сайт.
...
- The themes people can pick this month: Теми. Tick "Предлага се този месец" on the ones you offer. The same list feeds `/join` and the picker in `/account`.
...
Picks lock automatically 3 weeks before the delivery day each month, so production has time to make the shirts. There is nothing to set per month — only the delivery day itself (Сайт), which stays the same until you change it.

## Money

Everything about cards, refunds and cancellations happens in the Stripe dashboard or in the Customer Portal the customer reaches from `/account`. The site only records what Stripe reports through the webhook. If a subscription looks wrong on the site, check Абонаменти against Stripe; the Stripe side is the truth.
